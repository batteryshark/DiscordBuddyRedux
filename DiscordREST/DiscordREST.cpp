...
int GetAppAssets(const char* assets_info_url, unsigned long long application_id, const char* token, cJSON** info_response) {
	...
	memset(auth_header, 0, sizeof(auth_header));
	if (!res) {
		OutputDebugStringA("[DiscordBuddy] REST Error: GetAppAssets");
		return 0;
	}
...
int GetAppIDToken(unsigned long long app_id, char* discord_token, char* app_token) {
	...
	if (!res) {
		printf("Error on Discord App Token Request\n");
		return 0;
	}
...
int GetToken(const char* login_url, const char* totp_url, char* discord_token) {
	...
	int res = HTTPSPostJSON(login_url, NULL, login_request, &login_response);
	...
	if (cJSON_GetObjectItemCaseSensitive(login_response, "token")->valuestring) {
		...
		printf("Enter your Discord 2FA Code: ");
		...
		cJSON* totp_response;
		int res = HTTPSPostJSON(totp_url, NULL, totp_request, &totp_response);
